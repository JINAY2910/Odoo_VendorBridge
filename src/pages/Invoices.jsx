...
const Invoices = () => {
  ...
  const [selectedEmailInvoiceId, setSelectedEmailInvoiceId] = useState(null);
  ...
  const handleSendEmailSubmit = async (e) => {
    ...
    try {
      await api.post(`/invoices/${selectedEmailInvoiceId}/send-email`, { email: recipientEmail });
                  ...
                  invoices.map((i) => {
                    ...
                    <tr key={i._id} className={i.status === 'Paid' ? 'opacity-50 cursor-not-allowed bg-slate-50' : 'hover:bg-slate-50 transition-colors'}>
                      ...
                      <td className="px-6 py-4 text-right space-x-1.5 flex justify-end items-center">
                        ...
                        <button
                          onClick={() => {
                            setSelectedEmailInvoiceId(i._id);
                            setRecipientEmail('');
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md border border-slate-100 overflow-hidden transform transition-all">
            <div className="p-6 bg-slate-900 text-white flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Send PDF document directly to recipient</p>
              </div>
              <button 
            ...
            <form onSubmit={handleSendEmailSubmit} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Recipient Email Address</label>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  ...
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl text-sm shadow-md shadow-blue-600/10 transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
